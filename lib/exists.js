 ...
 */
export function exists(source, value, every) {
    if (isString(source)) {
        ...
        if (is(value, 'string')) {
            ...
            return every ? value.every((i) => exists(source, i)) : value.some((i) => exists(source, i));
        }
        ...
        if (isFunction(value)) {
            ...
            return every ? value.every((item) => exists(source, item)) : value.some((item) => exists(source, item));
        }
        else if (isObject(value)) {
            return source.some((a) => compare(a, value));
        }
        else {
